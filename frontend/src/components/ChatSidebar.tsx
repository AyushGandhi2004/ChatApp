...
import { CornerDownRight, CornerUpLeft, LogOut, MessageCircle, Plus, Search, UserCircle, UserCircleIcon, X } from 'lucide-react';
  ...
  return (
    <aside className={`fixed z-20 sm:static top-0 left-0 h-screen w-80 bg-gray-900 border-r border-gray-700 transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} sm:translate-x-0 transition-transform duration-300 flex flex-col`} >
                ...
                showAllUsers ? <div className="space-y-4 h-full">
                        <div className="relative">
                            <Search className="absolute left-3 top-2
                             transform translate-y-1/2 w-4 h-4 text-gray-400"/>
                        ...
                        <div className="space-y-2 overflow-y-auto h-full pb-4">
                            {
                                users?.filter((u)=> u._id !== loggedInUser?._id && u.name.toLocaleLowerCase().includes(searchQuery.toLowerCase())).map((u)=>(
                                    <button onClick = {()=>createChat(u)} key={u._id} className="w-full text-left p-4 rounded-lg border border-gray-700 hover:border-gray-600 hover:bg-gray-600 transition-colors">
                                        <div className="flex items-center gap-3 ">
                                            <div className="relative flex">
                                                <UserCircle className="w-6 h-6 text-gray-400"/>
                                                {/* Online Status */}
                                            {
                                                onlineUsers.includes(u._id) && <span className='absolute -top-0.5 -right-0.5 w-3.5 h-3.5 rounded-full bg-green-500 border-2 border-gray-900'/>
                                            }
                                            </div>
                                        ...
                                        <div className='flex items-center gap-3'>
                                            ...
                                            <div className="flex-1 min-w-0 ">
                                                    <div className="flex items-center justify-between">
                                                        ...
                                                        {
                                                            unseenCount > 0 && <div className="bg-red-600 text-xs text-white font-bold rounded-full min-w-[22 px] h-5.5 flex items-center justify-center px-2">
        ...
        <div className="p-4 border-t border-gray-700 space-y-2 ">
            ...
            <button onClick={handleLogout} className='w-full flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-gray-800 transition-colors hover:text-white'>
