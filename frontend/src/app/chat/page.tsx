...
const ChatApp = () => {
  ...
  useEffect(() => {
    ...
    const handleMessageSend = async(e : any , imageFile? : File | null)=>{
        ...
        try {
            ...
            setMessages((prev)=>{
              const curMessage = prev || [];
              const messageExist = curMessage.some((msg)=>msg._id === data.message._id)

              if(!messageExist) {
                return [...curMessage, data.message]
              }
              return curMessage;
            })
            setMessage("")
  ...
  return (
    <div className="min-h-screen flex bg-gray-900 text-white relative overflow-hidden">
        <ChatSidebar  sidebarOpen={sideBarOpen} setSidebarOpen={setSideBarOpen} showAllUsers={showAllUsers} setShowAllUsers={setShowAllUsers} users={users} loggedInUser={loggedInUser} chats={chats} selectedUser={selectedUser} setSelectedUser={setSelectedUser} handleLogout={handleLogout} createChat={createChat}
        onlineUsers = {onlineUsers}/>

        <div className="flex-1 flex flex-col justify-between p-4 backdrop-blur-xl bg-white/5 border border-white/10 ">
            <ChatHeader user={user} setSidebarOpen={setSideBarOpen} isTyping={typing} onlineUsers={onlineUsers}/>
